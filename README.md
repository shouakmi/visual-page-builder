...
> the selection. Structural drag (E2) is underway — its decision layer, a headless `@vpb/interaction`
> that computes where a drop lands, is done and tested; wiring it to pointer events in the app is the
> remaining step. The canvas renders through **the same compiler the export will call**, so what you
...
packages/
  ...
  interaction/          @vpb/interaction — drag/drop/snap geometry. Headless: no React, no DOM (E2)
...
| `interaction` | node     | Drag geometry: where a drop lands, composed with `moveNode` | 12 tests     |
...
| `web`      | jsdom       | Canvas wiring (D4), selection + overlay (E1), the `index.html` contract | 21 tests |

```bash
pnpm test                        # everything (851 today)
...
hit-test handle, the click→node walk, multi-select, the overlay), and 4 for E2 (drag geometry: the
axis, the before/after boundary, the index).
...
| E     | Interaction: overlay, structural drag, resize, multi-select, snap guides. **Selection (E1) ✅; drag geometry (E2) ✅, drag wiring next** |
