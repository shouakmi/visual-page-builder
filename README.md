...
> the selection. The canvas renders through **the same compiler the export will call**, so what you see
...
```bash
pnpm test                        # everything (839 today)
...
`present` vs `committed`, the active page, the per-page node filter, device sizing), and 7 for E1 (the
hit-test handle, the click→node walk, multi-select, the overlay).
...
| E     | Interaction: overlay, structural drag, resize, multi-select, snap guides. **Selection (E1) ✅; drag next** |
